    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("TagTrigger") && IsOwner) // reduntant check for future proofing
        {
            ...
            if(!blocked && otherState != thisState)
            {
                ...
                thisMovementController.PushAwayFromTagged(other.gameObject.GetComponentInParent<Transform>().position, transform.parent.GetComponent<NetworkObject>().OwnerClientId);
